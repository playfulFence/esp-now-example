...
async fn run(mut esp_now: EspNow, mut _icm : Icm42670<I2cProxy<'static, NullMutex<I2C<'static, hal::peripherals::I2C0, >>>>) {
    ...
    loop { 
        let accel = _icm.accel_norm().unwrap();

        println!("Sending accel {:x?}", accel);

        // Dealing with each axis
        let x_bits: u32 = accel[0].to_bits();
        let y_bits: u32 = accel[1].to_bits();
        let z_bits: u32 = accel[2].to_bits();

        let accel_bytes: [u8; 9] = [(x_bits >> 24) as u8, (x_bits >> 16) as u8, (x_bits >> 8) as u8,
                                    (y_bits >> 24) as u8, (y_bits >> 16) as u8, (y_bits >> 8) as u8,
                                    (z_bits >> 24) as u8, (z_bits >> 16) as u8, (z_bits >> 8) as u8];
    

        println!("Sending accel (bytes): x: {:x?}, y: {:x?}, z: {:x?}", [(x_bits >> 24) as u8, (x_bits >> 16) as u8, (x_bits >> 8) as u8],
                                                                        [(y_bits >> 24) as u8, (y_bits >> 16) as u8, (y_bits >> 8) as u8],
                                                                        [(z_bits >> 24) as u8, (z_bits >> 16) as u8, (z_bits >> 8) as u8]);
        ...
        let res = select(ticker.next(), async {
            ...
            if r.info.dst_address == BROADCAST_ADDRESS {
                ...
                #[cfg(any(feature = "esp32c3", feature = "esp32c2"))]
                esp_now.send(&r.info.src_address, &accel_bytes).unwrap();
            }
        })
        .await;
